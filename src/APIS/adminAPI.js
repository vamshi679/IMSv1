...
adminExpApp.post('/register', stdId, (req, res) => {
    dbo.collection('collection1').findOne({ email: req.body.email }, (err, studentobj) => {
        if (err) {
            console.log('err in find', err)
        }
                ...
                if (err) {
                    ...
                    dbo.collection('collection1').insertOne(req.body, (err, result) => {
                        if (err) {
                            ...
                            //     text: `username & password same ${req.body.imsid}`
                            // };
                            // // Step 3
...
adminExpApp.put('/update', (req, res) => {
    ...
    dbo.collection('collection1').findOne({ email: req.body.email }, (err, result1) => {
        if (err) {
            console.log('err in find', err)
        }
            ...
            dbo.collection('collection1').updateOne({ email: req.body.email }, {
                $set: {
